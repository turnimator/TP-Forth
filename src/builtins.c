...
long d_tos(ftask_p task) {
    return task->d_stack[task->d_top - 1];
}
...
inline int v_tos(ftask_p task) {
    return task->v_stack[task->v_top - 1];
}
...
static inline void d_true(ftask_p task) {
    d_push(task, 0);
}

static inline void d_false(ftask_p task) {
    d_push(task, 1);
}
...
*/
static void d_r(ftask_p task)
{
	r_push(task,(p_code_p*)d_pop(task));
}

static void r_d(ftask_p task)
{
	d_push(task,(long)r_pop(task));
}
