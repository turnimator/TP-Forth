...
inline p_code_p* p_code_here(ftask_p task){
	return task->pcp;
}
...
long offs(program_p prog, p_code_p* pc){
	return pc-prog->pcp_array;
}
...
void run_task(ftask_p task) {
    run_prog(task, task->program);
}
